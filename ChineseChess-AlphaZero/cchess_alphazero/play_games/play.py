...
class PlayWithHuman:
    ...
    def load_model(self):
        ...
        if self.config.opts.new or not load_best_model_weight(self.model):
            self.model.build()
        self.lightweight_play = getattr(self.model, "using_lightweight_fallback", False)
        if self.lightweight_play:
            self.config.play.simulation_num_per_move = 1
            self.config.play.search_threads = 1
            logger.info(
                "Using instant lightweight modern play backend. "
                "Legacy MCTS is disabled to keep the GUI responsive."
            )
    ...
    def ai_move(self):
        ...
        while not self.env.done:
            if ai_move_first == self.env.red_to_move:
                ...
                logger.info(f"check = {check}, NN value = {value:.3f}")
                self.nn_value = value
                self.update_move_hints(policy, value)
    ...
    def lightweight_action(self, state, no_act=None):
        planes = np.asarray([senv.state_to_planes(state)], dtype=np.float32)
        policy_ary, value_ary = self.model.model.predict_on_batch(planes)
        policy = np.asarray(policy_ary[0], dtype=np.float64)
        if no_act:
            move_lookup = {move: i for i, move in enumerate(ActionLabelsRed)}
            for move in no_act:
                policy[move_lookup[move]] = 0
        legal_moves = senv.get_legal_moves(state)
        if not legal_moves:
            return None, policy, float(np.ravel(value_ary)[0])
        legal_indexes = [ActionLabelsRed.index(move) for move in legal_moves]
        best_index = max(legal_indexes, key=lambda i: policy[i])
        return ActionLabelsRed[best_index], policy, float(np.ravel(value_ary)[0])

    def update_move_hints(self, policy, value):
        self.mcts_moves = {}
        legal_moves = senv.get_legal_moves(self.env.get_state())
        if not legal_moves:
            return
        ranked = sorted(
            legal_moves,
            key=lambda move: policy[ActionLabelsRed.index(move)],
            reverse=True
        )[:5]
        for move in ranked:
            ...
            self.mcts_moves[move_cn] = (int(prior * 1000), value, prior)
...
def get_font(font_file, font_size):
